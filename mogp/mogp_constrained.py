...
#
#     # X1 = np.vstack(
#     #     [np.random.uniform(-3., 1., (4, 4)), np.random.uniform(-2., -1., (3, 4)), np.random.uniform(-1., 1., (10, 4)),
#     #      np.random.uniform(1., 3., (3, 4))])
#     # X2 = np.vstack(
#     #     [np.random.uniform(-4., 1., (6, 4)), np.random.uniform(-2., -1., (6, 4)), np.random.uniform(-1., 1., (10, 4)),
#     #      np.random.uniform(1., 3., (8, 4))])
#     #
#     # Y1 = np.sin(X1) + np.random.randn(20, 4) * 0.5
#     # Y2 = np.sin(X2 + 20) + np.random.randn(30, 4) * 0.5
#     # X = np.vstack([X1, X2])
#     # Y = np.vstack([Y1, Y2])
#
#     # Clustered version of data - 3 groups of linear slopes - does MoGP recreate this?
#     np.random.seed(0)
#     X_slow = np.sort(np.vstack(
#         [np.random.uniform(0., 7., (5, 4)), np.random.uniform(3., 6., (10, 4)), np.random.uniform(4., 7., (5, 4))]))
#     X_med = np.sort(np.vstack([np.random.uniform(0., 2., (10, 4)), np.random.uniform(1., 4., (10, 4))]))
#     X_fast = np.sort(np.vstack([np.random.uniform(0., 2., (20, 4))]))
#
#     noise_scal = 5
#     Y_slow = -7 * X_slow + 48 + np.random.randn(X_slow.shape[0], 4) * noise_scal
#     Y_med = -13 * X_med + 48 + np.random.randn(X_med.shape[0], 4) * noise_scal
#     Y_fast = -24 * X_fast + 48 + np.random.randn(X_fast.shape[0], 4) * noise_scal
#     X = np.vstack([X_slow, X_med, X_fast])
#     Y = np.vstack([Y_slow, Y_med, Y_fast])
#     Xi_new = np.array([0.25, 0.5, 1, 2.4])
#     Yi_new = np.array([40, 35, 30, 30])
#     yorg_std = np.std(Y)
#     yorg_mean = np.mean(Y)
#
#     norm_test = True
#
#     if norm_test:
#         Yi_new = (Yi_new - yorg_mean) / yorg_std
#         Y = (Y - yorg_mean) / yorg_std
#         mix = MoGP_constrained(X=X, Y=Y, alpha=1, num_iter=1,  mean_func=True, signal_variance=1., signal_variance_fix=True,
#                            noise_variance=0.5, noise_variance_fix=False, kernel='rbf', normalize=False)
#
#     else:
#         mix = MoGP_constrained(X=X, Y=Y, alpha=1, num_iter=1,  mean_func=True, signal_variance=1., signal_variance_fix=True,
#                                noise_variance=0.5, noise_variance_fix=False, kernel='rbf', normalize=True)
#
#     mix.sample()
#     p = mix.predict(Xi_new, Yi_new)
#     print(p)
#     pclust = np.argmax(p[:-1])
#     print(pclust)
#     ypred = mix.obsmodel[pclust].model.predict(Xi_new.reshape(-1,1))[0]
#
#     if norm_test:
#         ypred = ypred*yorg_std + yorg_mean
#
#     print(ypred)